...
[`01-spec-controller-overview.md`](./controller/01-spec-controller-overview.md); for the optimizer it builds on, see
[`01-spec-optimizer-overview.md`](./optimizer/01-spec-optimizer-overview.md).
...
weather feed ([P3 §3](./optimizer/03-spec-optimizer-digital-twin.md#1-the-forward-model)). Phase 4
...
This extends the HAL **coupling matrix** ([P1 §3](./controller/03-spec-controller-hal-simulation.md))
...
- It runs **downstream of the control loops** (which still compute desired heat/CO₂ demand) and
  **upstream of safety interlocks and actuator constraints** — selection decides *which* actuators
  ...
  ([P1 §7](./controller/06-spec-controller-safety-and-constraints.md#2-safety-interlocks),
  [P1 §9](./controller/06-spec-controller-safety-and-constraints.md#4-actuator-constraints)).
...
- **Manual override and safety still win.** An operator override forces a specific actuator state
  ahead of selection ([P1 §10](./controller/02-spec-controller-architecture.md#6-manual-override)); safety interlocks
...
still planned independently ([P3 §1](./optimizer/01-spec-optimizer-overview.md)). The feed is the
...
([P3 §7](./optimizer/04-spec-optimizer-planning.md#2-optimization-objectives)) into a genuine **combinatorial
...
| **TimescaleDB** | Phase 2 store → optimizer | Unchanged — read-only history per greenhouse ([P3 §8](./optimizer/09-spec-optimizer-interfaces.md)) |
...
| Introducing the crop → targets mapping | Still **owned by Phase 2** ([P2 crop profiles](./platform/05-spec-platform-crop-profiles.md)); Phase 4 plans only **within** the crop-safe bounds |
