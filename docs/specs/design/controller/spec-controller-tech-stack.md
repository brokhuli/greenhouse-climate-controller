...
> [constraints](./spec-controller-constraints.md). Host tooling (broker install,
> editors) is in
...
- **Why:** Fixed by [tech-stack-decisions.md](../tech-stack-decisions.md#phase-1--deterministic-greenhouse-controller).
  ...
  [explicit stage interfaces](./spec-controller-architecture.md#5-module-composition-rules)
  (`P1-MAINT-1`).
...
- **How:** Serves the
  [controller REST surface](./spec-controller-interfaces.md#3-rest--the-sole-write-path);
...
- **Why:** TOML is the chosen config format
  ([config](./spec-controller-config-and-parameters.md)); `serde` + `toml` map it to
...
- **What:** The temperature [PID controller](./spec-controller-control-loops.md#fast-loops--reactive).
...
- **How:** Implemented in the control-loop module with integral clamping; gains come
  from [config](./spec-controller-config-and-parameters.md#default-parameters-reference).
...
- **Why:** `P1-TEST-1` requires ≥ 90% line coverage on the control-loop and
  ...
  ([HAL §7](./spec-controller-hal-simulation.md#7-determinism--seeding)).
...
- **An embedded/`no_std` or RTOS target** — the HAL is pure simulation and never
  runs on a device ([constraints](./spec-controller-constraints.md#5-hal-swappable--no-real-hardware));
