...
| `503 Service Unavailable` | Phase 2 could **not establish a baseline** — a cold-start greenhouse whose controller is unreachable to seed one — so **nothing was recorded** ([contract](../../../../contracts/optimizer-platform-setpoints-rest/components/responses.json)). The offline-but-recorded case is a `202`, not a `503`. | Not owned by Phase 2; the cycle is held and retried next cadence rather than claiming state Phase 2 does not hold. | `escalated` · `platform_unavailable` (transient) |
...
| Rule | Behavior |
|---|---|
| **Single-flight per greenhouse** | At most one cycle is in flight per greenhouse. The fixed cadence ([planning](./04-spec-optimizer-planning.md#1-llm-driven-planning)) plus a per-greenhouse in-flight guard means a slow cycle (near the [P3-PERF-2](../../artifacts/non-functional-requirements.md) 90 s bound) finishes or times out **before** the next begins — there is never an optimizer-vs-optimizer race on the write path. N greenhouses plan **concurrently** — the scheduler runs their cycles in parallel, bounded by `max_concurrent_cycles` ([architecture — scheduling](./02-spec-optimizer-architecture.md#scheduling-concurrent-per-greenhouse-single-flight), [P3-SCAL-1](../../artifacts/non-functional-requirements.md)); single-flight is **per greenhouse, not global**, so the parallelism runs *across* greenhouses while each greenhouse still serializes its own successive cycles. |
...
Each applied bundle carries its `optimizer_run_id` as provenance — transported on the
`X-Optimizer-Run-Id` request header of the `POST /setpoints` write (an auxiliary trace header that
never affects the write outcome; a malformed value is ignored), which Phase 2 records on the
resulting `source = optimizer` intended-state revision
([RFC-005](../../../decisions/request-for-comments.md#rfc-005-setpoint-authority-and-delivery-chain),
