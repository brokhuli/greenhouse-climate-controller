...
- **How:** The planner is the chain `ChatPromptTemplate | LLM | StructuredOutputParser`
  ...
  pinned by `prompt_version` — the prompt analog of the pinned `.python-version` and `provider`
  ...
  config change + restart); switching the **`model`** within the active provider's allowlist is instead an
  **operator runtime action** ([interfaces](./10-spec-optimizer-interfaces.md#service-api-endpoints)), each
  allowlisted model held to its own pre-captured baseline. Any configured fallback
