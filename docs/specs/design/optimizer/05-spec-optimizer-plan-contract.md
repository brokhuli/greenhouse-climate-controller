...
[`contracts/optimizer-internal-plan-schema/`](../../../../contracts/optimizer-internal-plan-schema/) and is catalogued in
...
| Field | Type | Required | Description |
|---|---|---|---|
| `at` | `string` (RFC 3339, UTC) | ✓ | The instant these targets apply. `trajectory[0].at` is the cycle's start; later optional change points are strictly ordered, remain within the horizon, and land on whole-hour offsets from that start. |
...
Per the **self-contained-contract convention**, `contracts/optimizer-internal-plan-schema/` carries a **local copy** of
this shape (as the [write-path contract](../../../../contracts/optimizer-platform-setpoints-rest/) already does)
...
[setpoint write path](../../../../contracts/optimizer-platform-setpoints-rest/)). Its `schema_version` therefore
