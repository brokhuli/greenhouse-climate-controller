...
/** House-level `temperature`/`humidity`/`co2` readings keep the fleet card's metric tiles live. */
export function applyTelemetryToSummary(
  ...
  const co2 = frame.readings.find((reading) => reading.metric === "co2");
