...
```json
{
  "trajectory": [
    ...
    { "at": "2026-06-17T13:00:00Z", "setpoints": { "co2_target_ppm": 1050, "vpd_target_kpa": 0.95 } }
  ],
  "immediate_setpoints": { "temperature_day_c": 23.0, "co2_target_ppm": 1000, "vpd_target_kpa": 0.9 },
  "confidence": 0.9,
  "explanation": "Pre-position CO2 and VPD for the morning ramp while holding DLI within band.",
...
3. **Never emit an empty patch (`{}`).** Every `setpoints` object — each trajectory point and
   `immediate_setpoints` — must set at least one field. If an hour changes nothing, repeat the prior
   hour's targets rather than emitting an empty object.
